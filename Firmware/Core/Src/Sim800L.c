...
void Init_Sim_Parameter(void )
{
	...
	hsimcom.sim_stat=SIM_UNKNOWN;
	char data_in[SIMCOM_DATA_IN_MAX_SIZE];
	hsms.sms_inited=-1;
}
void simcom_puts(uint8_t* pData)
{
	HAL_UART_Transmit(&huart3, pData, strlen((char*)pData), 3000);
}
void turnOnSimUart(void)
{
	HAL_UART_Receive_IT(&huart3, &hsimcom.data_in_tmp, 1);
}
	...
	do	
		{ 
			while(Transfer_cplt2 ==0)
			{
				...
				{
					if (HAL_UART_Receive_IT(&huart3, &hsimcom.data_in_tmp, 1) == HAL_OK)
					{
						time1 = HAL_GetTick();
					}
					else 
